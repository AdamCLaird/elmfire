...
! Ref: https://doi.org/10.1016/j.proci.2024.105755
...
FTP_PA = BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%FTP_CRIT                  ! Critical flux time product
...
! Ref: https://doi.org/10.1016/j.proci.2024.105755 Supp. Materials
...
IF (V_MPS .LT. 10) THEN  ! LOW WIND SPEED (<10 m/s)

   ! Linear correlation coefficients with house size and separation distance
   D1 = 1.679463256 - 0.123901243*HAMADA_A + 0.307612446*HAMADA_D    ! α₁; eqn S4
   D2 = 78.62957398 + 1.536189561*HAMADA_A - 0.5662073*HAMADA_D      ! α₂; eqn S5

   S1 = -2.922896622 - 0.05550541*HAMADA_A + 0.017291361*HAMADA_D    ! β₁; eqn S6
   S2 = 39.31478699 + 0.768094781*HAMADA_A - 0.28310365*HAMADA_D     ! β₂; eqn S7

   U1 = -6.297892493 - 0.119654483*HAMADA_A + 0.037754535*HAMADA_D   ! γ₁; eqn S8
   U2 = 78.62957398 + 1.536189561*HAMADA_A - 0.5662073*HAMADA_D      ! γ₂; eqn S9

   ! Linear correlations with wind speed
   C%ELLIPSE_PARAMETERS%DIST_DOWNWIND = C%WIND_PROP*(D1*V_MPS + D2)  ! downwind flame size , a (m); eqn S1
   C%ELLIPSE_PARAMETERS%DIST_UPWIND = C%WIND_PROP*(U1*V_MPS + U2)    ! sidewind flame size , b (m); eqn S2
   C%ELLIPSE_PARAMETERS%DIST_SIDEWIND = C%WIND_PROP*(S1*V_MPS + S2)  ! upwind flame size,    c (m); eqn S3

ELSEIF (V_MPS .GT. 17.3) THEN  ! HIGH WIND SPEED (>17.3 m/s)

   ! Linear correlation coefficients with house size and separation distance
   D1 = -7.159031537 - 0.043555289*HAMADA_A - 0.14894238*HAMADA_D    ! α₁; eqn S4
   D2 = 394.4930697 + 0.720929023*HAMADA_A + 11.42149084*HAMADA_D    ! α₂; eqn S5

   S1 = -0.577270631 - 0.015285438*HAMADA_A + 0.012786629*HAMADA_D   ! β₁; eqn S6
   S2 = 38.11784939 + 0.800599307*HAMADA_A - 0.412476476*HAMADA_D    ! β₂; eqn S7

   U1 = -1.092711783 - 0.025390239*HAMADA_A + 0.016740663*HAMADA_D   ! γ₁; eqn S8
   U2 = 52.39584604 + 1.104793131*HAMADA_A - 0.57241037*HAMADA_D     ! γ₂; eqn S9
...
ELSE  ! MEDIUM WIND SPEED (10<V_MPS<17.3 m/s)
   
   ! Linear correlation coefficients with house size and separation distance
   D1 = 4.099488028 - 0.000767118*HAMADA_A + 0.134372426*HAMADA_D    ! α₁; eqn S13
   D2 = -94.26651508 - 0.000694022*HAMADA_A - 3.053034015*HAMADA_D   ! α₂; eqn S14
   D3 = 615.192675 + 0.300438559*HAMADA_A + 19.34120221*HAMADA_D     ! α₃; eqn S15

   S1 = 0.437844987 + 0.008280661*HAMADA_A - 0.002833081*HAMADA_D    ! β₁; eqn S16
   S2 = -10.13978982 - 0.192922421*HAMADA_A + 0.067862023*HAMADA_D   ! β₂; eqn S17
   S3 = 66.32382799 + 1.282260348*HAMADA_A - 0.484673257*HAMADA_D    ! β₃; eqn S18

   U1 = 0.525004045 + 0.01046073*HAMADA_A - 0.004473105*HAMADA_D     ! γ₁; eqn S19
   U2 = -12.4091466 - 0.249326233*HAMADA_A + 0.109759448*HAMADA_D    ! γ₂; eqn S20
   U3 = 84.64808209 + 1.727651884*HAMADA_A - 0.801945211*HAMADA_D    ! γ₃; eqn S21

   ! Quadratic correlations with wind speed
   C%ELLIPSE_PARAMETERS%DIST_DOWNWIND = C%WIND_PROP*(D1*V_MPS**2 + D2*V_MPS + D3)   ! downwind flame size , a (m); eqn S10
   C%ELLIPSE_PARAMETERS%DIST_UPWIND = C%WIND_PROP*(U1*V_MPS**2 + U2*V_MPS + U3)     ! sidewind flame size , b (m); eqn S11
   C%ELLIPSE_PARAMETERS%DIST_SIDEWIND = C%WIND_PROP*(S1*V_MPS**2 + S2*V_MPS + S3)   ! upwind flame size,    c (m); eqn S12

ENDIF

! ----------------------------------------------------------------------------------------

! Calculate urban DFC ellipse geometry
...
! Ref: https://doi.org/10.1071/WF24102 
! Ref: https://doi.org/10.1016/j.proci.2024.105755

USE ELMFIRE_VARS

TYPE(NODE), POINTER, INTENT(INOUT) :: BURNING_NODES                           ! Pointer to NODE representing burning cell
REAL ::  BURNING_TIME, EARLY_TIME, HRR_PEAK, DEVELOPED_TIME, DECAY_TIME
REAL, INTENT(IN) :: T                                                         ! Current simulatin time (s)

EARLY_TIME = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%T_EARLY         ! Early stage duration (s)
DEVELOPED_TIME = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%T_FULLDEV   ! Fully developed stage duration (s)
DECAY_TIME = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%T_DECAY         ! Decay stage duration (s)
HRR_PEAK = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%HRRPUA_PEAK       ! Peak heat release rate per unit area (W/m2)

IF (BURNING_NODES%IFBFM .NE. 91) HRR_PEAK = BURNING_NODES%FLIN_SURFACE / ANALYSIS_CELLSIZE

BURNING_TIME = T - BURNING_NODES%TIME_OF_ARRIVAL                              ! Cell burning duration (s)

IF (BURNING_TIME .LE. EARLY_TIME) THEN
   BURNING_NODES%HRR_TRANSIENT = (HRR_PEAK/ EARLY_TIME)*BURNING_TIME          ! Linearly grow to peak HRR
ELSEIF ((BURNING_TIME .GT. EARLY_TIME) .AND. (BURNING_TIME .LE. DEVELOPED_TIME)) THEN
   BURNING_NODES%HRR_TRANSIENT = HRR_PEAK                                     ! Peak HRR
ELSEIF (BURNING_TIME .GT. DECAY_TIME) THEN
   BURNING_NODES%HRR_TRANSIENT = 0.                                           ! All fuel consumed
ELSE
   BURNING_NODES%HRR_TRANSIENT = (HRR_PEAK/(DEVELOPED_TIME - DECAY_TIME))*(BURNING_TIME - DECAY_TIME) ! Linearly decay to zero
ENDIF
